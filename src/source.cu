...
{
    unsigned int hits[MAX_COLLISIONS];
    int count;
};
                                 ...
                                 int nTriangles)
{
    ...
    if (i < nTriangles) {
        ...
        for (int c = 0; c < 3; c++) {
            ...
            vC[c] = static_cast<coord_t>((centroid + half_delta[c]) * inv_delta[c]);
        }
        //- compute morton code
...
__device__ morton_t inline highestBit(int i, morton_t *morton)
...
__device__ void bvhUpdateParent(BVHNode* node, BVHNode* internalNodes,
                                BVHNode *leafNodes, morton_t *morton, int nNodes)
{
